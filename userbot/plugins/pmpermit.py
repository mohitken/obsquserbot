...
USER_BOT_WARN_ZERO = "`kyne :`This is {DEFAULTUSER}'s userbot kyne! "
USER_BOT_NO_WARN = ("[░█─░█ █▀▀ █── █── █▀▀█\n░█▀▀█ █▀▀ █── █── █──█\n░█─░█ ▀▀▀ ▀▀▀ ▀▀▀ ▀▀▀▀](tg://user?id=724495167)\n\n"
...
if Var.PRIVATE_GROUP_ID is not None:
    ...
    async def approve_p_m(event):
        ...
        if len(approved_users) > 0:
            ...
            APPROVED_PMs = "`kyne :`No Approved PMs (yet)"
...
async def hehehe(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            pmpermit_sql.approve(chat.id, "`kyne :`User approved you to PM")
            await borg.send_message(chat, "`kyne :`User has come!")
